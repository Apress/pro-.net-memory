...
{
    class PinneableObjectCacheUsage : ICollectScenario
    {
        public int Run()
        {
            // Cleanup everything
            ...
            PinnableBufferCache cache = new PinnableBufferCache("Test", () => new PinableObject());
            var obj1 = (PinableObject)cache.Allocate();
            var handle1 = GCHandle.Alloc(obj1.F1, GCHandleType.Pinned);
            var obj2 = (PinableObject)cache.Allocate();
            var handle2 = GCHandle.Alloc(obj2.F1, GCHandleType.Pinned);
            Console.WriteLine(obj1.F1);
            Console.WriteLine(obj2.F1);
            cache.Free(obj1);
            cache.Free(obj2);
            return 0;
        }
